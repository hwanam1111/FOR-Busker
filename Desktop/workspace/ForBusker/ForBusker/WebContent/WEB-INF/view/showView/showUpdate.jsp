...
<%
	...
	ShowVO sVO = (ShowVO)request.getAttribute("showVO");
	String shDateTime = sVO.getShDate()+" "+sVO.getShTime();
...
$(function(){
	...
	var shType= "<%=sVO.getShType()%>";
	
	if(shType!="노래"&&shType!="댄스"&&shType!="연주"&&shType!="마술"&&shType!="퍼포먼스"){
		$("#ShowRegistGenreEtcInput").show();
		$("#ShowRegistGenreInput").val("기타").attr("selected","selected");
		$("#ShowRegistGenreEtcInput").val(shType);	
	}else{		
	$("#ShowRegistGenreInput").val(shType).attr("selected","selected");
	}
...
<!-- ################################################### -->
	...
	<!-- 이부분 부터 코딩 시작 -->
	<div class="hoc">
		<form id="form1" runat="server" method='post' style="margin-left:140px;" action="showUpdateOk.do">
			<table>
				<!-- 어떤 입력폼인가 -->
				<tr>
				...
				<tr>
					<div class="form-group row">
						...
						<div class="col-xs-8">
							<input class="form-control" type="text" placeholder="제목을 입력하세요" id="ShowRegistTitleInput" name="shName" value=${showVO.shName }>
						</div>
					</div>
				</tr>
				<!-- 영상URL -->
				<tr>
					<div class="form-group row">
						...
						<div class="col-xs-8">
							<input class="form-control" type="text" placeholder="홍보 영상을 업로드하세요" id="ShowRegistVideoInput" name="shVideo" value=${showVO.shVideo }>
				...
				<tr>
					...
					<div class="col-xs-6">
						<input class="form-control" type="text" id="ShowRegistAddrInput" name="shMapAddr" placeholder="주소검색을 눌러주세요." 
						value= "<%=sVO.getShMapAddr()%>" style="background-color: #fff;" readonly/>
				...
				<tr>
					<div class="form-group row">
						...
						<div class="col-xs-8">
					   <textarea class="form-control" id="ShowRegistLocDetailInput" name="shMapDetail" rows="2" 
					   placeholder="위치에 대한 설명을 해주세요." ><%=sVO.getShMapDetail() %></textarea>
					</div>
				</tr>
				<!-- 시간선정 -->
				<tr>
					<div class="form-group row">
						...
						<div class="col-xs-4">
							<input type="text" class="some_class form-control" placeholder="시간을 정해주세요 " id="ShowRegistTimeInput" 
							name="shDateTime" value="<%=shDateTime %>"/>
				...
				<tr>
					<div class="form-group row">
						...
						<div class="col-xs-8">
							<input class="form-control" type="text" placeholder="공연자 이름(팀 이름)을 입력하세요" id="ShowRegistPerformerInput" name="shTeamName" value=${showVO.shTeamName }>
						</div>
					</div>
				</tr>
				<!-- 공연분류-->
				...
				<tr>
					<div class="form-group row">
					   ...
					   <textarea class="form-control" id="ShowRegistShowDetail" rows="5" name="shDetail"
    					placeholder="공연에 대한 설명을 해주세요." ><%=sVO.getShDetail() %></textarea>
						</div>
					</div>
				</tr>
				<tr>
					<div align="center">
						<button type="button" id="submitbtn" class="btn btn-info">수정하기</button>
					</div>
				</tr>
			</table>
				<input type="hidden" id="ShowRegistAddrCoords" name="shMapCoords" value="<%=sVO.getShMapCoords() %>" />
				<input type="hidden"  name="shNo" value="<%=sVO.getShNo() %>" />
